...
class Coder {
  constructor(meta) {
    if (meta.bytes)
      throw Error('Unsupported bytes mode');
    this._ = meta.bytes && '$' || '_';
    this.m = meta;
    this.turns = [];
  ...
  bits$code(e, depth) {
    let x = e.value,
      script = genCall('EatBits', `0b${x},${x.length}`);
      if (isOnce(e)) return script;
      return repeat(e.min, e.max) + func(script, '', depth) + ', 0)';
  }

  bits_code(e, depth) {
    throw Error('Unsupported bits for string mode');
  }

  codes$code(e, depth) {
    let
      script = !e.isRange &&
      genCall('EatBytes', e.value.map(toHex).join(',')) ||
      genCall('EatRangeByte', e.value.map(toHex).join(','));

    if (!e.value.every(function(x) {return x >= 0 && x <= 255;}))
      throw Error('Unsupported codes.value greater than 0xFF for byte mode');
  ...
  unicode(args) {
    if(this.m.bytes)
      throw Error(`Unsupported to--unicode for bytes mode`);
...
module.exports = function(metadata) {
  let
    ...
    script = new Coder(metadata).forms.map(function(script, i) {
      return func(script, `${i}: ${this[i]}`, 0);
    }, formulas).join(',\n  ');

  metadata.formulas = null;
  config = JSON.stringify(metadata, null, '  ');

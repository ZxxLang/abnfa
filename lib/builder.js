...
class Builder {
  ...
  parse(source) {
    ...
    if (this.isPartial())
      throw Error(`Not exactly matched: ${this.offset}/${this.srcLength}`);
  ...
  create(typeIndex, recordIndex) {
    ...
    if(decl.kind === 'struct') {
      let
        n = bare();
      if (this.coder.typefield)
        n[this.coder.typefield] = this.types[typeIndex];
      if (this.coder.locfield)
        n[this.coder.locfield] = this.locate(this.record[recordIndex]);
      return n;
    }
  ...
  refer_FLOAT(field, decode) {
    return this.decode('FLOAT', field, decode || 'string');
  }
  ...
  direct_fault(message, key) {
    ...
    if (this.coder.mode === 'string') {
      let i = this.src.indexOf(this.crlf || ' ', at);
      if (i !== -1) {
        if (i > 80 + at) i = at + 20;

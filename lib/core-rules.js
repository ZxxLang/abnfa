...
Rules.prototype.addRef = function(ref) {
	...
	if (action.method || action.key) {
		var method = action.method || '';

		if (action.flag) method += '-' + action.flag
		if (action.key) method += (action.flag && '-' || '--') + action.key
...
Rules.prototype.bare = function bare() {
	...
	n.type = this.types.slice(0).sort();
...
function Action(actions) {
	var s = actions.shift();
	if (s) this.type = s;
	if (s = actions.shift()) this.method = s
	if (s = actions.shift()) this.key = s
	if (actions.length) this.flag = actions.shift()
}
...
function Ref(tok, rep) {
	...
	// ref-[to]-[key]- ===> ref-ref-[to]-[key]
	if (actions[actions.length - 1]) actions.shift()
	if (actions.length)
...
Ref.prototype.bare = function bare() {
	...
	if (this.action) n.action = this.action.bare()

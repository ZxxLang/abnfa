...
function clone(src) {
	...
	var dst = create()
	for (var k in src) dst[k] = clone(src[k]);
	return dst
}
...
function Rules() {
	...
	//     rep: repeat,
	//     radix: 2 | 10 | 16,
	//     fields: [ String( 1*BIT | 1*DEC | 1*HEX ) ]
	// }
	//
	// BIT  "0" | "1"
	// DEC  "0"-"9"
...
Rules.prototype.addRef = function(ref) {
	...
	if (action) {
		...
		if (action.method || action.property) {
			var method = action.method || 'assign';

			if (action.extra) method += '-' + action.extra
			if (action.property) method += (action.extra && '-' || '--') + action.property

			a = this.actions[this.current] =
				this.actions[this.current] || create()

			a = a[method] = a[method] || create()
			a = a[name] = a[name] || []
			a.indexOf(ref.ref) == -1 && a.push(ref.ref)
		}
	}
...
Rules.prototype.bare = function bare() {
	...
	n.produce = this.produce.slice(0).sort();

	n.actions = clone(this.actions)
...
function Ref(tok, rep) {
	var action = tok.raw.split('-');
	if (rep) this.rep = rep
	this.ref = action.shift()
	if (action.length) {
		this.action = create()
		this.action.produce = action.shift()
		if (action.length) this.action.method = action.shift()
		if (action.length) this.action.property = action.shift()
		if (action.length) this.action.extra = action.join('-')
	}
}
...
Ref.prototype.bare = function bare() {
	...
	if (action) {
		n.action = create()
		if (action.produce != null) n.action.produce = action.produce;
		if (action.method != null) n.action.method = action.method;
		if (action.property != null) n.action.property = action.property;
		if (action.extra != null) n.action.extra = action.extra;
	}
...
function Range(tok, rep, radix) {
	...
	this.radix = radix
	this.first = mm[0].toLowerCase()
	this.last = mm[1].toLowerCase()
}

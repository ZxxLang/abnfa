...
module.exports = function(meta) {
  ...
  meta.types = types.map((t, i) => {
    if (!t.kind || t.kind === 'interface') {
      ...
      return t; // interface and enmu
    }

    if (t.kind !== 'struct') return t;

    let decl =  {kind: 'struct', fields: Object.create(null)};
    for (let k in t.fields) {
      if (decl.fields[k]) throw Error(
        `Duplicate ${typenames[i]}.${k} definitions`
        );

      if (t.fields[k].kind !== 'mixins') {
        decl.fields[k] = t.fields[k];
        continue;
      }
      ...
      for (let key in mix.fields) {
        ...
        decl.fields[key] = mix.fields[key];
      }
    }
    return decl;
  }).map((t) => {

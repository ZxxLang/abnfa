...
require('./patternize')(md.formnames,md.formulas);
  ...
  seq(lit('to-'), alt(
    seq(
      lit('mode'), more('WSP'), alt(
        seq(lit('string'), 'to--STRING(mode)'),
        seq(lit('byte'), 'to--STRING(mode,byte)'),
        seq(lit('bits'), 'to--STRING(mode,bits)'),
...
rule('field-annotation',
  seq(
    lit('%d'), more('DIGIT--STRING(value)'), 'to--STRING(kind, INT)'
  ),
  seq(
    lit('\''),

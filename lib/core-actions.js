...
function nextOperand(list, i, typ) {
	var a;
	for (; i < list.length; i++) {
		a = list[i]
		if (a && a.method != 'note' &&
			(a.factors || typ || a.type && a.type[0] != '-'))
			return i
	}
	return -1
}
...
Actions.prototype.action = function action(n) {
	...
	if (n.method == 'next') {
		if (n.type) {
			this.err = 'unexpected type on ' + n.ref + n.tail
			return false
		}
		...
		list = this.factors
		x = nextOperand(list, i, n.type)

		if (x == -1 || start == this.pos || i == list.length) {

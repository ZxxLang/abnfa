...
Plugins.CORE_ACTIONS = function CORE_ACTIONS(n, self) {
	if (!n.method || self.pos) return false
...
Plugins.LOAD_OUTDENT = function LOAD_OUTDENT(n, self) {
	var key = n.key == 'SP' && 'S' || 'T';
	if (!self.plugins.CRLF && !Plugins.LOAD_CRLF(n, self))
		return false
	self.plugins.OUTDENT = Plugins.OUTDENT.bind(key)
	self.plugins.ON_OUTDENT = Plugins.ON_OUTDENT.bind(key)
	return true
}

Plugins.OUTDENT = function OUTDENT(n, self) {
	return self.before('OUTDENT',
		n.method == 'allow' && [] || null)
}

Plugins.ON_OUTDENT = function ON_OUTDENT(self, list, i, n) {
	var line = n.loc.startLine,
		col = n.loc.startCol,
		allow = n.extra;
	if (i) return false;
	i++;
	while (i < list.length) {
		n = list[i++]
		if (!n || n.loc.startLine == line || n.loc.startCol > col)
			continue
		if (n.loc.startCol < col || !allow || !allow.indexOf(i - 1) + 1) {
			self.err = 'outdent at ' +
				n.loc.startLine + ':' + n.loc.startCol
			return false;
		}
	}

	if (allow) {
		list = self.upperFactors()
		n = list && list[0]
		if (self.isEvent(n, 'OUTDENT')) {
			if (!n.extra) n.extra = [];
			n.extra.push(i)
		}
	}
...
Actions.prototype.before = function before(key, extra) {
	...
	event.key = 'ON_' + (key || '')
	if (extra)
		event.extra = Array.isArray(extra) && extra || [extra]
...
function buildInfix(a, j) {
	...
	s = o.extra
	if (n.extra || s) o.extra = n.extra || null
	if (s || n.extra) n.extra = s || null
...
Actions.prototype.create = function create(n, start, raw) {
	...
	if (n) {
		...
		if (n.extra) a.extra = n.extra.slice();
	}
...
Actions.prototype.repeat = function repeat(n) {
	...
	while (!this.err && c != max &&
		this.pos < this.src.length && method.call(this, n)) c++;

...
Actions.prototype.markEvent = function markEvent(n, key, extra) {
	var event = this.create(n, this.stage.pos, false, this.stage.line, this.stage.col)
	event.type = 'ACTIONS'
	event.method = 'mark'
...
Actions.prototype.play = function play(n) {
	return n && this.repeat(n) && this.build(this.factors) ||
...
function buildFactors(list, infix) {
	...
	while (i < list.length) {
		...
		if (p.method == 'infix') {
			infix.push([i - 1, list[innerPrecedence(list, i)].precedence])
			continue
		}

		if (!p.type || p.factors) continue

		if (p.method == 'ahead') {
			e = i - 1
			while (e && !list[--e]);
			p.factors = [list[e]]
			p.start = list[e].start
			list[e] = null
		}

		e = p.end
		while (i < list.length) {
			if (e <= list[i].start) break
...
Actions.prototype.action = function action(n) {
	...
	if (i + 1 == list.length) {
		// num   = 1*DIGIT-Number
		list[i] = this.create(n, start, true)
		return true
	}
	...
	if (n.method == 'ahead') {
		this.stage.flags |= AHEAD
		return true
	}

	if (n.method == 'prefix') {
		x = innerOperand(list, i + 1)
		if (x == -1) {
			while (i != list.length) list.pop();
			return false
		}
		...
		asso.factors = []
		x = i + 1
		while (x < list.length && list[x].end <= asso.end)
			asso.factors.push(list[x++]);
		while (x < list.length) list[++i] = list[x++];
		x = x - i
		while (--x) list.pop();
		return true
	}
	...
	if (n.method == 'infix') {
		x = innerPrecedence(list, i + 1)
		if (x != -1) {
			x = innerOperand(list, x + 1)
			if (x != -1) {
				asso.end = list[x].end
				this.stage.flags |= INFIX
				return true
			}
		}

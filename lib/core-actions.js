...
function failed() {
	...
	this.failNode.src = JSON.stringify(
		this.src.substring(
...
function emitEvents(ths, list, events) {
	return events.every(function(n) {
		return this.plugins['ON_' + n.type.substring(1)](this, list, n)
	}, ths)
}
...
Actions.prototype.repeat = function repeat(n) {
	...
	while (!this.err && c != max && method(n, this)) {
	...
	if (!this.err && c >= (!n.rep && 1 || n.rep.min)) {
		this.pos = last
		if (this.pos >= this.frozen) return true;
	}

	this.pos = pos

	if (!this.err && pos < this.frozen)

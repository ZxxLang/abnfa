...
Plugins.FLAG = function FLAG(n, self) {
	...
	if (!a || a.type && a.type[0] == '-') {
		self.err = 'missing an Action before FLAG' + (n.tail || '')
		return false
	}
	...
	if (!n.tail) {
		...
		if (a.flag) {
			self.err = 'FLAG must be before FLAG-flags'
			return false
		}
		a.flag = '+'
		return true
...
Actions.prototype.play = function play(n) {
	...
	if (!this.err && !ok) return new Error(
		'Actions match failed at: ' + failed.call(this))

	if (!this.err && this.pos != this.end) return new Error(
		'Actions are not fully matched at: ' + failed.call(this))

	return !this.err && this.build(this.factors) || new Error(
		'Actions match failed, ' + this.err + ': ' + failed.call(this))
}
...
function buildFactors(list) {
	...
	while (i < list.length) {
		...
		if (x == -1) return 'missing an Action on ahead'
...
Actions.prototype.action = function action(n) {
	...
	if (!n.method && !n.type) {
		...
		for (x = i; x < list.length; x++)
			if (!list[x].type || list[x].type[0] != '-') {
				asso = list[x]
				break
			}

		if (!asso) {
			this.err = 'missing an Action in ' + n.ref + n.tail
			return false
		}
	...
	if (n.method == 'next') {
		...
		// non-event, allow note
		for (x = i; x < list.length; x++) {
			if (list[x].type && list[x].type[0] == '-') {
				this.err = 'unexpected an event action in ' + n.ref + n.tail
				return false
			}
			if (list[x].method != 'note') {
				asso = list[x]
				break
			}
		}
		...
		// exchange list[i], list[x]
		while (x != i) list[x] = list[--x];
		list[i] = asso
		return true
	}
	...
	if (n.method == 'leaf' || n.method == 'note') {
		...
		if (i == list.length) {
			...
			asso = list[i]
			if (i + 1 != list.length || asso.raw == null) {
				this.err = 'expected the only lit Action in ' + n.ref + n.tail
				return false
			}
	...
	if (n.method == 'precedence') {
		if (!n.key || n.type) {
			this.err = 'expected key only on ' + n.ref + n.tail
			return false
		}

		if (!this.repeat(rule) || start == this.pos) return false
		...
		if (!asso.precedence) {
			this.err = 'missing precedence on ' + n.ref + n.tail
			return false
		}
	...
	if ((!n.type || !n.key) && n.method == 'ahead') {
		this.err = 'unsupported action on ' + n.ref + n.tail
		return false
	}

...
function toTok(start, formal, raw) {
    var tok = {
        formal: formal,
        raw: raw,
        start: {
    ...
    for (i = 0; i < tokens.length;) {
        ...
        for (j++; j < tokens.length; j++)
            if (tokens[j].start.col <= rule.start.col &&
                ['w', 'c'].indexOf(tokens[j].formal) == -1) break
...
function tokenize(s, ...collector) {
    ...
    // ''     '%d'                                      ; can be ignore
    // ''     '%x'                                      ; can be ignore
    // 'w'    1*(1*%x09 / 1*%x20)                       ; can be ignore
    // 'c'    (%x0D %x0A) / %x0D / %x0A                 ; can be ignore
    //
    // 'r'    ALPHA *(ALPHA / DIGIT)                    ; rulename or references
    // '='    "="                                       ; defined
    // ';'    ";" *(WSP / VCHAR)                        ; comment
    // '*'    (*HEXDIG "*" *HEXDIG) / 1*HEXDIG          ; repeat
    //
    // 'a'    rulename 1*3('-' [rulename])              ; action
    // 's'    *(%x20-7E)                                ; case-sensitive string
    // 'i'    *(%x20-7E)                                ; case-insensitive
    ...
    while (i < s.length) {
        ...
        if (c == ' ' || c == '\t') {
            ...
            if (c == '<') {
                while (prose(s.charCodeAt(l))) l++;
                if (s[l] != '>') return helper.syntaxError(
                        'expected < *(%x20-3D / %x3F-7E) >',
                        s.slice(i).split(crlf, 1)[0], start);
            } else {
                if (c == '"')
                    while (dquote(s.charCodeAt(l))) l++;
                else
                    while (quote(s.charCodeAt(l))) l++;
                if (s[l] != c)
                    return helper.syntaxError(c == '"' &&
                    'expected DQUOTE *(%x20-21 / %x23-7E) DQUOTE' ||
                    'expected QUOTE *(%x20-26 / %x28-7E) QUOTE',
                        s.slice(i).split(crlf, 1)[0], start);
            }
            prob = collector.retrans(toTok(start, '', c)) ||
            collector.retrans(toTok(start, c == '"' && 'i' || 's', s.slice(i + 1, l))) ||
            collector.retrans(toTok(start, '', c == '<' && '>' || c))
            ...
            c = s[l + 1]
            if (c != 'b' && c != 'd' && c != 'x')
                return helper.syntaxError('expected %b|%d|%x',
                    s.slice(l).split(crlf, 1)[0], start)

...
var test = require('./test'),
	...
	grammarCRLF = [
		'first  = ACTIONS-EOF ACTIONS-CRLF 1*(*CWSP i-lit-) EOF',
		'i      = "i"/"h"',
		common,
	].join('\n'),
	grammarFLAG = [
	...
	grammarPythonIndent = [
		...
		'else       = "else:" *SP (',
		'           	expression-factors-orelse /',
		'               stmts-factors-orelse )',
...
test('python indent', function(t) {
	...
	[
		...
		['if [t]:\n\tdo', 'if[~test[List[~elts[Ident"t"]]],~body[Ident"do"]]'],
		...
		['if t: do\nelif x:do',
			'if[Ident~test"t",~body[Ident"do"],~orelse[' +
			'if[Ident~test"x",~body[Ident"do"]]' +
			']]'
		],
		['if t:do \nelif t: do\nelse:\n\ti\n\tx\ndo()',
			'if[Ident~test"t",~body[Ident"do"],~orelse[' +
